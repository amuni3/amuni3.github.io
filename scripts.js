...
/** Author strings in publications.json that should get .highlight-name (accent color + weight). */
...
function isHighlightedAuthor(name) {
  return HIGHLIGHT_AUTHORS.has(String(name).trim());
}
...
function createPublicationElement(publication) {
  ...
  // Add authors with highlight (edit HIGHLIGHT_AUTHORS above for alternate spellings)
  const authors = document.createElement('div');
  authors.className = 'pub-authors';

  publication.authors.forEach((author, index) => {
    if (isHighlightedAuthor(author)) {
